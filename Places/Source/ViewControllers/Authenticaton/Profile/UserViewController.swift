...
class UserViewController: UIViewController, DataSourceProviderDelegate {
    ...
    var userImageView = UIImageView(image: Asset.person.image)
    ...
    func didSelectRowAt(_ indexPath: IndexPath, on tableView: UITableView) {
        ...
        guard let action = AuthUserAction(rawValue: actionName) else {
    ...
    private func configureNavigationBar() {
        ...
        navigationBar.addProfilePic(userImageView)
    }
    
    private func updateUserImage() {
        guard let photoURL = user?.photoURL else {
            let defaultImage = Asset.personCircleFill.image
            userImageView.image = defaultImage
            return
        }
        userImageView.setImage(from: photoURL)
    }
    ...
    private func adjustUserImageAlpha(_ offset: CGFloat) {
        ...
        userImageView.alpha = 1 - (verticalOffset * 0.05)
    }
}
